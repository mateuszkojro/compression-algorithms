...
void Harness::RunOverDataset(
  ...
  for (auto length: config_.text_lengths) {
    ...
    if (length > dataset_text.size()) {
      std::clog << "Testing size: " << length << " exceeded dataset: " << dataset_name << " size ("
                << dataset_text.size() << ") -- skiping" << std::endl;
      continue;
    }

    for (auto&[name, executor]: executors_) {
      std::clog << "Running: " << dataset_name
